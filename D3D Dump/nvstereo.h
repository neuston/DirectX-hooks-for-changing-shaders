...
namespace nv {
    namespace stereo {
		...
		struct D3D11Type
		{
			...
			static void UpdateTextureFromStaging(Device* pDevice, Context* pContext, Texture* tex, StagingResource* staging)
			{
				pContext; tex; staging;
				...
				pContext->CopySubresourceRegion(tex, 0, 0, 0, 0, staging, 0, &stereoSrcBox);
			}
		};
        ...
        public:
            ...
            void UpdateStereoTexture(Device* dev, Context* con, Texture* tex, bool deviceLost)
            {
                if (!mInitialized) {
                    Init(dev);
                }
                ...
                if (staging) {
                    D3DType::UpdateTextureFromStaging(dev, con, tex, staging);
                    staging->Release();
                }
            }
